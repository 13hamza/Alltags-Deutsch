...
- **Words page (`words.html`)** — Comprehensive vocabulary directory grouped alphabetically (A–Z) with CEFR level filters and live search. The **All** tab combines A1, A2 and B1 (each word shown once, at its lowest level).
- **Vocabulary Test (`test.html`)** — A fresh exam every time for **A1, A2 and B1**, in **30, 50 or 100 marks**. Up to nine question types (fill in the blanks, multiple choice, reverse choice, der/die/das, correct sentence, sentence meaning, matching, translation, true/false) in a shuffled part order. Answers never follow a fixed pattern: true/false is balanced and never repeats the previous paper's sequence, and correct options are spread evenly over A–D.
...
- **Stories & Story reader (`stories.html` & `story.html`)** — Graded German short stories (A1, A2 and B1) with an interactive word lookup glossary. Click any word in a story to see its translation and hear it pronounced.
- **Topics & Topic page (`topics.html` & `topic.html`)** — 64 topics organised by level — **A1** (most common, everyday essentials), **A2** (less common, weekly/monthly) and **B1** (life admin, work, opinions) — and grouped by theme inside each level (see `TOPIC_ORGANISATION` in `data.js`). Topic-by-topic breakdowns featuring vocabulary tickets and interactive real-life dialogues line-by-line with a full dialogue playback feature.
